...
/// Light control: Off / On / Programmé, with schedule editing when the
/// scheduled mode is selected. Hours are edited locally and pushed with
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 12) {
                ZStack {
                    Circle()
                        .fill(pool.lightOn ? Color.yellow : Color.secondary.opacity(0.25))
                        .frame(width: 38, height: 38)
                    Image(systemName: pool.lightOn ? "lightbulb.fill" : "lightbulb")
                        .font(.system(size: 17, weight: .medium))
                        .foregroundStyle(pool.lightOn ? .white : .secondary)
                }
            ...
            if pool.lightSelection == .scheduled {
                HStack(spacing: 16) {
